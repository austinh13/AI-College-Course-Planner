...
export default function ReviewStep({ profile, constraints, academicHistory, overrides, onOverridesChange, manualElectiveEntry, onManualElectiveEntryChange, onEdit, onContinue }) {
  ...
  // `overrides` and `manualElectiveEntry` are lifted to App so they
  // survive this component unmounting when the user navigates to another
  // screen and back (see setOverrides/setManualElectiveEntry below).
  const setOverrides = (updater) => onOverridesChange(typeof updater === "function" ? updater(overrides) : updater);
  ...
  if (loadError) {
    return (
      <div className="mx-auto flex w-full max-w-[640px] flex-col gap-7">
        <Eyebrow boxed={false}>Recommended courses</Eyebrow>
        ...
        <button type="button" className="mt-1 inline-flex w-fit items-center justify-center rounded-full border border-[#1f5c43] bg-transparent px-8 py-4 text-md text-[#f2f5f3] transition-colors duration-200 hover:border-[#8a8d8f]" onClick={onEdit}>
  ...
  return (
    <div className="mx-auto flex w-full max-w-[640px] flex-col gap-7">
      <Eyebrow>Recommended for next term</Eyebrow>
      <h1 className="m-0 text-display font-bold leading-[1.08] tracking-[-0.02em] text-[#f2f5f3]" style={{ fontFamily: "var(--font-display)" }}>
        Here's what we'd
        <br />
        take next.
      ...
      {shortfallHours > 0 && (
        <p className="rounded-2xl border border-[#e87500]/25 bg-[#e87500]/10 px-4 py-3 text-base text-[#f5d5b2]">
      ...
      <div className="flex flex-col gap-3.5">
        {slotPicks.map((slot) => {
          ...
          return (
            <div className="flex flex-col gap-2 rounded-panel border border-white/10 bg-[#0d1b18]/80 p-4 shadow-[0_18px_36px_rgba(0,0,0,0.18)]" key={slot.groupKey}>
              <span className="text-xs font-medium uppercase tracking-[0.08em] text-[#5fe0b7]" style={{ fontFamily: "var(--font-mono)" }}>
              ...
              {codes.map((code, i) => {
                ...
                return (
                  <div className="flex flex-wrap items-center gap-2.5" key={i}>
                    <select
                      className="min-w-0 flex-1 rounded-2xl border border-white/10 bg-[#081712] px-3 py-2.5 text-base text-[#f2f5f3] transition-all focus:outline-none focus:ring-2 focus:ring-[#5fe0b7]/30"
                      value={code}
                    ...
                    <span className="text-sm text-[#8a8d8f]" style={{ fontFamily: "var(--font-mono)" }}>{course.hours} SCH</span>
                    {codes.length > 1 && (
                      <button
                        type="button"
                        className="h-9 w-9 rounded-full border border-white/10 bg-[#0c1715] text-md leading-none text-[#8a8d8f] transition-colors hover:border-white/20 hover:text-[#f2f5f3]"
                    ...
                    {!ok && (
                      <p className="mt-1 basis-full text-sm text-[#e87500]">
              ...
              {canAddMore && (
                <button type="button" className="self-start bg-transparent p-0 text-sm font-medium text-[#5fe0b7] hover:underline" onClick={() => addSlotPick(slot)}>
                  + Add another class
                </button>
              )}
            </div>
        ...
        {electivePicks.map((group) => {
          ...
          return (
            <div className="flex flex-col gap-2 rounded-panel border border-white/10 bg-[#0d1b18]/80 p-4 shadow-[0_18px_36px_rgba(0,0,0,0.18)]" key={group.groupKey}>
              <span className="text-xs font-medium uppercase tracking-[0.08em] text-[#5fe0b7]" style={{ fontFamily: "var(--font-mono)" }}>
                {group.sectionTitle === "Core Curriculum Requirements" ? "Core elective" : "Elective"} · {group.label}
              </span>
              {isManual ? (
                <Input
                  className="border-white/10 bg-[#081712] px-3 py-2.5 text-base text-[#f2f5f3] placeholder:text-[#8a8d8f]"
                  type="text"
                ...
                <select
                  className="rounded-2xl border border-white/10 bg-[#081712] px-3 py-2.5 text-base text-[#f2f5f3] transition-all focus:outline-none focus:ring-2 focus:ring-[#5fe0b7]/30"
                  value={selectedCode}
              ...
              {options && (
                <button
                  type="button"
                  className="self-start bg-transparent p-0 text-sm text-[#5fe0b7] hover:underline"
                  onClick={() =>
              ...
              {!ok && <p className="text-sm text-[#e87500]">Prerequisite not yet satisfied for {selectedCode}.</p>}
            </div>
          );
        })}
      </div>
      ...
      {(remainingSlots.length > slotPicks.length || remainingElectiveGroups.length > electivePicks.length) && (
        <details className="text-base text-[#8a8d8f]">
      ...
      <div className="mt-1 flex flex-wrap gap-3">
        <Button type="button" variant="outline" className="rounded-full border border-white/10 bg-transparent px-8 py-4 text-md text-[#f2f5f3] transition-colors duration-200 hover:border-white/20 hover:bg-white/5" onClick={onEdit}>
          Edit academic history
        </Button>
        <Button
          type="button"
          size="lg"
          className="rounded-full border border-transparent bg-[#e87500] px-8 py-4 text-md font-semibold text-[#081712] transition-transform duration-150 enabled:hover:translate-y-[-1px] disabled:cursor-not-allowed disabled:opacity-40"
